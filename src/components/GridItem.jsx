...
const GridItem = ({ item,location,imageUrl, year, type, rating, title, onDelete }) => {
  ...
  return (
    <div className="me-4 md:me-10 my-4 relative cursor-pointer group" onClick={()=>handleOnClick(item.id,type)}>
      <div className='bg-tertiary rounded-md'>
        <img src={item.backdrop_path?imageUrl:placeholder} alt={title} className="rounded-md w-[160px] md:h-[150px] md:w-[250px]" />
      ...
      <div className="text-xs md:text-[0.8rem] text-gray-400 px-2 pt-1">
        ...
        <div className='inline'>
          <img src={type=='movie'?reel:tv} alt="type" className='h-3 inline me-1' />{type}
        </div>
        <span> • </span>
        <span>{rating}</span>
      </div>
